...
xports.getWishlist = (req, res, next) => {
  if (req.role === "user") {
    User.findOne({ email: req.email })
      .then((data) => {
        ...
        res
          .status(200)
          .json({ data: "your wish list is here", wishlist: data.wishlist });
      })
      .catch((err) => next(err));
  }
};
    ...
    User.findOne({ email: req.email })
      .then((data) => {
        ...
        if (data.wishlist.includes(req.body.wishlist))
          throw new Error("it's already in your wishlist");
        User.updateOne(
          { email: req.email },
          {
      ...
      .then((data) => {
        ...
        User.updateOne(
          ...
          if (data.modifiedCount)
            throw new Error(
              "this product is already removed from your wishlist"
            );
          res
            .status(200)
            .json({ data: "removed from wishlist", wishlist: data });
        });
      })
      .catch((err) => next(err));
  }
